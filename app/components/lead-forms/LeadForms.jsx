...
export default function LeadForms({
  ...
  const fetchLeadForms = async () => {
    try {
      ...
      const response = await api.leadForms.getAll(currentPage, 20, filters);
      console.log(response);

      if (response.success) {
        setLeadForms(response.data.leadForms);
  ...
  const handleViewDetails = async (leadForm) => {
    try {
      if (role === 'user') {
        const response = await api.leadForms.update(leadForm._id, {
          assignee: user._id,
          status: 'active',
        });
        ...
        if (response.success) {
          ...
          console.log('Successfully assigned lead to yourself');
        }
      }
      console.log('View details for:', leadForm._id);
    } catch (error) {
  ...
  const handleDialingStatusChange = async (leadFormId, newStatus) => {
    try {
      const response = await api.leadForms.update(leadFormId, {
    ...
    <>
      {loading ? (
        ...
        <div className="bg-transparent rounded-lg flex flex-col flex-1 h-[calc(100vh-20rem)]">
          {/* Table Header */}
          <div className="grid grid-cols-6 gap-4 p-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs xl:text-sm font-semibold min-w-[800px] rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="border-r border-white/20 md:pr-4">Card Number</div>
            <div className="border-r border-white/20 md:pr-4">Zip Code</div>
            <div className="border-r border-white/20 md:pr-4">
              Dialing Status
            </div>
            <div>Actions</div>
            ...
            {leadForms.length > 0 ? (
              leadForms.map((leadForm) => {
                ...
                return (
                  <div
                    key={leadForm._id}
                    className="grid grid-cols-6 gap-4 p-4 text-gray-300 text-xs xl:text-sm hover:bg-gray-800/50 transition-colors 
                  ...
                  >
                    ...
                    <div className="border-r border-white/10 pr-4">
                      {canSee
                        ? leadForm.phone
                        : `${leadForm.phone?.slice(
                            0,
                            3
                          )} ${leadForm.phone?.slice(3, 6)} •••• ••••`}
                    </div>
                    <div className="border-r border-white/10 pr-4">
                      {canSee ? (
                        <span className="font-mono tracking-wider">
                          {leadForm.cardNumber?.replace(
                            /(\d{4})(?=\d)/g,
                            '$1 '
                          )}
                        </span>
                      ) : (
                        `${leadForm.cardNumber?.slice(
                          0,
                          4
                        )} ${leadForm.cardNumber?.slice(4, 6)}•• •••• ••••`
                      )}
                    </div>
                    <div className="border-r border-white/10 pr-4">
                      {canSee ? leadForm.zipCode : `••••`}
                    </div>
                    <div className="border-r border-white/10 pr-4">
                      <select
                    ...
                    <div className="flex justify-center items-center">
                      ...
                      <Button
                        onClick={() => {
                          ...
                          console.log('Action for:', leadForm._id);
                        }}
                        disabled={
