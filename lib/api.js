// Note: `${env}/api` is always truthy, so the old `|| fallback` never applied.
// Fall back to localhost only when the env var is actually missing.
...
export const auth = {
  login: async (email, password) => {
    ...
    console.log('[API] login() otpRequired =', response?.otpRequired);

    // Agents get an OTP step (no token yet); admins get a token immediately
    if (response.success && response.data?.token) {
      console.log('[API] login() token received -> admin path');
...
export const users = {
  ...
  create: async (userData) => {
    return await apiCall('/users', {
      method: 'POST',
      body: userData, // Let apiCall handle FormData (avatar upload) vs JSON
    });
  },
...
export const leadForms = {
  ...
  // Accepts a single File or an array/FileList of Files
  bulkUpload: async (files) => {
    const formData = new FormData();
    const fileList = Array.isArray(files) ? files : Array.from(files ?? [files]);
...
export const userFiles = {
  // Upload one or more files for a specific user.
  // Accepts a single File or an array/FileList of Files.
  upload: async (userId, files) => {
    const formData = new FormData();
    const fileList = Array.isArray(files) ? files : Array.from(files ?? [files]);
...
export const sheets = {
  getAll: async (page = 1, limit = 50, filters = {}) => {
    ...
    return await apiCall(`/sheets?${params.toString()}`);
  },

  getById: async (id) => {
    return await apiCall(`/sheets/${id}`);
  },
